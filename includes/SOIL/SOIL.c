	...
	)
{
	...
	if( tex_id )
	{
		...
		} else
		{
			...
			unsigned int clamp_mode = SOIL_CLAMP_TO_EDGE; // GL_CLAMP;
	...
	for( cf_target = ogl_target_start; cf_target <= ogl_target_end; ++cf_target )
	{
		if( (int)(buffer_index + DDS_full_size) <= buffer_length )
		{
			...
			if( uncompressed )
			{
				...
				for(unsigned index = 0; index < DDS_full_size; index += block_size )
				{
					unsigned char temp = DDS_data[index];
					DDS_data[index] = DDS_data[index + 2];
					DDS_data[index + 2] = temp;
				}
				glTexImage2D(
					cf_target, 0,
